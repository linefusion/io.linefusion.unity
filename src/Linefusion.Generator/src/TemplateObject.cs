...
namespace Linefusion.Generator
{
    ...
    {
        public static ScriptObject Wrap<T>()
        {
            ...
            foreach (var methods in type.GetMethods(BindingFlags.Public | BindingFlags.Static).Where(m => !m.IsSpecialName).GroupBy(method => method.Name))
            {
                ...
                {
                    return type.InvokeMember(methods.Key, BindingFlags.Public | BindingFlags.Static | BindingFlags.InvokeMethod, Type.DefaultBinder, null, parameters);
                });
            }
            ...
            foreach (var methods in type.GetMethods(BindingFlags.Public | BindingFlags.Instance).Where(m => !m.IsSpecialName).GroupBy(method => method.Name))
            {
                ...
                {
                    Debug.WriteLine("Invoking method " + methods.Key);
                    ...
                    return type.InvokeMember(
                        methods.Key,
                        BindingFlags.Public | BindingFlags.Instance | BindingFlags.InvokeMethod | BindingFlags.Default,
                        Type.DefaultBinder,
                        instance,
                        methodParameters
                    );
